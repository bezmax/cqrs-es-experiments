...
class ProcessManagerSpec extends Specification {
    ...
    def "potential stop loss higher for 10 seconds -> move stop loss up"() {
        ...
        0 * publisher.publish(_ as StopLossPriceUpdated)
    ...
    def "price is lower than stop loss for 13 seconds -> notify about stop loss hit"() {
        ...
        agent.receive(new PriceUpdated(price: 930))
        then:
        ...
        0 * publisher.publish(_ as StopLossHit)
    ...
    def "message received after stop loss hit -> ignore it"() {
        when:
        agent.receive(new PositionAcquired(price: 1000))
        then:
        1 * publisher.publish({ StopLossPriceUpdated msg -> msg.price == 950 })

        when:
        agent.receive(new PriceUpdated(price: 900))
        agent.receive(new PriceUpdated(price: 800))
        then:
        _*publisher.publish(_ as SendToMeIn)

        when:
        agent.receive(new RemoveFrom13SecWindow(price: 900))
        then:
        1 * publisher.publish({ StopLossHit msg -> msg.price == 950 })

        when:
        agent.receive(new RemoveFrom13SecWindow(price: 800))
        then:
        0 * publisher.publish(_ as StopLossHit)
